...
function selectedMonth(){return currentDate.toISOString().slice(0,7)}
...
function saldoPrevistoProximoMes(){
  ...
  const mes=proximoMes.toISOString().slice(0,7);
...
const balance=income-spent-totalGuardadoMetas;
 ...
 const prevD=state.despesas.filter(
   x=>isSameMonth(x.data,prev.toISOString().slice(0,7))
 );
...
pontuacao = Math.max(0, pontuacao);
  ...
  // Define a cor de fundo de acordo com a saúde financeira
...
if(pontuacao >= 80){
  ...
  document.body.classList.add('saude-boa');
...
function openIncomeModal(){
  ...
  const mesAtualSistema=hoje.toISOString().slice(0,7);
  const mesSelecionado=selectedMonth();
...
function openCategoriaModal(){openModal(`<button class="modal-close" onclick="closeModal()">×</button><h2>Nova categoria</h2><form onsubmit="addCategoria(event)"><div class="form-grid"><div class="field"><label>Nome</label><input id="cName" required placeholder="Ex.: Pets"></div><div class="field"><label>Ícone</label><input id="cIcon" value="📦" maxlength="2"></div></div><div class="actions"><button type="button" class="btn secondary" onclick="closeModal()">Cancelar</button><button class="btn">Salvar</button></div></form>`)}
